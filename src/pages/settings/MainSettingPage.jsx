...
const MainSettingPage = () => {
  ...
  const { data, isLoading: bannerLoading } = useQuery(['banner-main'], () => BannerService.info('main'));
    ...
    <React.Fragment>
      ...
      <SDataDetailBody padding>
        ...
        {body?.bannerType === 'image' && (
          <React.Fragment>
            ...
            <DataDetailItem label="(모바일)" span={2}>
              ...
              />
              <FileUpload ref={fileMobileRef} files={fileMobile} maxCount={1} onFileHandler={setFileMobile} folder="main" />
            </DataDetailItem>
          </React.Fragment>
        )}
        ...
        {body?.bannerType === 'video' && (
          <React.Fragment>
            <DataDetailItem label="(웹)" span={2}>
              <UploadBtn onClick={videoRef} helpText="권장 사이즈 : 1920*1080 / 최대 용량 : 5MB / 최대 개수 : 1개" />
              <VideoUpload ref={videoRef} files={video} maxCount={1} onFileHandler={setVideo} folder="main" />
            </DataDetailItem>
            ...
            <DataDetailItem label="(모바일)" span={2}>
              ...
              />
              <VideoUpload ref={videoMobileRef} files={videoMobile} maxCount={1} onFileHandler={setVideoMobile} folder="main" />
            </DataDetailItem>
          </React.Fragment>
        )}
      </SDataDetailBody>
