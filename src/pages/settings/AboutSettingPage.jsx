...
const AboutSettingPage = () => {
  ...
  const { data, isLoading: bannerLoading } = useQuery(['banner-about'], () => BannerService.info('about'));
    ...
    <React.Fragment>
      ...
      <SDataDetailBody padding>
        ...
        <DataDetailItem label="배너이미지" span={2}>
          <UploadBtn onClick={fileRef} helpText="권장 사이즈 : 1920*1080 / 최대 용량 : 5MB / 최대 개수 : 10개" />
          <FileUpload ref={fileRef} files={file} maxCount={10} onFileHandler={setFile} folder="main" />
        </DataDetailItem>
      </SDataDetailBody>
    </React.Fragment>
  );
