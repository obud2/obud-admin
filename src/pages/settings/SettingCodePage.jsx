...
const SettingCodePage = () => {
  ...
  const [codeBody, setCodeBody] = useState({ isOpen: false, type: '', data: '' });
  ...
  const onClickSubmit = () => {
    ...
    Promise.all([CodeService.saveItem('edit', param1), CodeService.saveItem('edit', param2)])
      .then(() => {
      ...
      <Flex gap="20px" flexDirection={matchese ? 'column' : 'row'}>
        <SDataDetailBody padding>
          ...
          <Card
            ...
            title="편의시설"
            bordered
            extra={<CardExtra title="추가하기" isLoading={isAllLoading} onClick={() => onClickOpenCodeList('convenience')} />}
          >
            <Typography.Text code type="secondary">
              수정 및 삭제를 하여도 해당 카테고리를 등록한 상품에는 수정 및 삭제가 되지않습니다.
            </Typography.Text>
          ...
          <Card type="inner" title="이용안내" bordered>
            <Typography.Text code type="secondary">
              공간등록 페이지 하단 "이용안내" 폼에 나타나는 문구입니다. 비워두면 나타나지 않습니다.
            </Typography.Text>
            <Spacing spacing="5" />
          ...
          <Card type="inner" title="환불규정" bordered>
            <Typography.Text code type="secondary">
              공간등록 페이지 하단 "환불규정" 폼에 나타나는 문구입니다. 비워두면 나타나지 않습니다.
            </Typography.Text>
            <Spacing spacing="5" />
        ...
        <SDataDetailBody padding>
          ...
          <Card
            ...
            title="분류"
            bordered
            extra={<CardExtra isLoading={isAllLoading} title="추가하기" onClick={() => onClickOpenCodeList('type')} />}
          >
            <CodeList
              list={body?.type || []}
              onClickEdit={onClickOpenCodeList}
          ...
          <Card
            ...
            title="카테고리"
            bordered
            extra={<CardExtra isLoading={isAllLoading} title="추가하기" onClick={() => onClickOpenCodeList('category')} />}
          >
            <Typography.Text code type="secondary">
              수정 및 삭제를 하여도 해당 카테고리를 등록한 상품에는 수정 및 삭제가 되지않습니다.
            </Typography.Text>
