...
const ProductStudioPage = () => {
  ...
  const { data: studio } = useQuery(["product-studio-detail", id], () =>
    ProductService?.getStudio(id)
  );

  const fetchData = async (cursor) => {
    let res = await ProductService?.getLessons(
      id,
      cursor?.pageParam,
      searchFilter?.value || ""
    );
    ...
    if (searchFilter?.filter) {
      res["value"] = res?.value.filter(
        (a) => a?.lessonType === searchFilter?.filter
      );
    }
  ...
  const {
    ...
    fetchNextPage,
    isFetchingNextPage,
  } = useInfiniteQuery(["product-studio-lesson-list", id], fetchData, {
  ...
  const onLessonDetail = (data) => {
    navigation(
      `/pages/product/studio/${data?.studiosId || ""}/${data?.id || ""}`
    );
  };
