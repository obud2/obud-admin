...
const AlimTalk = () => {
  ...
  const getList = () => {
    PortalService.getChannelList(ALLTALK_GROUP).then((result) => {
      ...
      result.value
        .sort((a, b) => a.id - b.id)
        .map((cate) => ch.push({ value: cate.id, label: cate.id, ft_api: cate.ft_api, at_api: cate.at_api }));
      setChannels(ch);
    });
  };
  ...
  const getTemplateList = (_channel) => {
    PortalService.getTemplateList(_channel).then((result) => {
      ...
      result
        ...
        .map((temp) => ch.push({ value: temp.kkoBizCode, label: temp.note, contents: temp.templateContents }));
      setTemplate(ch);
    });
  };
            ...
            <PhoneBox>
              ...
              <ScrollDiv className={'scroll-box'}>
                <div className={isWide ? 'phone-box-content wide' : 'phone-box-content'}>
                  ...
                  <div className={isWide ? 'preview-wrap wide' : 'preview-wrap'}>
                    {useImage > 0 && (
                      <div>
                  ...
                  {isAd === true && <div className="info ad-info">수신거부 | 홈 &gt; 채널차단</div>}
                </div>
              </ScrollDiv>
            </PhoneBox>
            <Divider dashed />
