...
const TemplateList = (props) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          {record.approval === 'Y' && (
            <Button type="primary" icon={<CloudUploadOutlined />} onClick={() => requestRelease(record)}>
              운영이관
            </Button>
          )}
        </Space>
  ...
  const requestRelease = (code) => {
    PortalService.requestRelease({ channelId: ALLTALK_CHANNEL, kkoBizCodes: [code.kkoBizCode] }).then((result) => {
      getList();
    });
  };
    ...
    <React.Fragment>
      ...
      <Descriptions size="small" column={1}>
        ...
        <Descriptions.Item label="">
          검수완료 : 검수가 완료된 상태입니다. 실제 사용하시려면 [운영이관]버튼을 클릭하여 사용요청해주세요.
        </Descriptions.Item>
      </Descriptions>
