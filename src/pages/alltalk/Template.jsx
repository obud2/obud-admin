  ...
  const getList = () => {
    PortalService.getChannelList(ALLTALK_GROUP).then((result) => {
      ...
      result.value
        .sort((a, b) => a.id - b.id)
        .map((cate) =>
          ch.push({ value: cate.id, label: cate.id, ft_api: cate.ft_api })
        );
      console.log(ch);
      setChannels(ch);
    });
  ...
  return (
    <React.Fragment>
      ...
      <Space direction={"vertical"} size={0}>
        ...
        <Text>
          {
            "가변 항목은 #{이름}, #{VAR1}, #{VAR2}으로 본문에 입력하시면 됩니다."
          }
        </Text>
      </Space>
        ...
        <Row gutter={100}>
          <Col span={12}>
            ...
            <Space direction="vertical">
              <Text type="secondary">
                적용 후 카카오 내부기준에 따라 심사과정에 의해 더 적절하다고
                판단되는 카테고리로 변경될 수 있습니다.{" "}
              </Text>
            </Space>
            ...
            <Space direction="vertical" size={0}>
              <Text type="secondary">
                부가 정보형, 복합형 선택시 부가정보 필수 입력{" "}
              </Text>
              <Text>
                부가정보는 변수 사용이 불가능하며, 부가정보 500자, 광고성 80자로
                각각 제한됩니다.
              </Text>
            </Space>
            ...
            {data.pointType === "img" && (
              <div>
                <Space direction="vertical" block size={0}>
                  ...
                  <Text>
                    지원 규격 : 최대 500kb / jpg, png 파일만 첨부 가능합니다.
                  </Text>
                  <Text>
                    이미지 사이즈는 800px * 400px (가로 500px 이상, 가로:세로
                    비율이 2:1이상 3:4이하만 가능)
                  </Text>
                  <Upload {...uploadProps}>
            ...
            {(data.templateType === "info" || data.templateType === "mix") && (
              <div>
                ...
                <div>
                  <Space direction="vertical">
                    <Text>
                      템플릿 내용과 부가정보를 함께 작성 시, 최대 1,000자까지
                      가능합니다.
                    </Text>
                  </Space>
                </div>
                  ...
                  <div className="talk-content-warp">
                    ...
                    {data.pointType === "title" && (
                      <div className="tit-point">
                        <span className="prew-txt1 subTitle">
                          강조표기 보조문구 미리보기
                        </span>
                        <strong className="prew-txt2 title">
                          타이틀 미리보기
                        </strong>
                      </div>
                    )}
                    ...
                    <p
                      className="txt contents"
                      dangerouslySetInnerHTML={{ __html: txt }}
                    ></p>

                    <div className="info-wrap">
                      {(data.templateType === "info" ||
                        data.templateType === "mix") && <p>{info}</p>}
                      {(data.templateType === "channel" ||
                        data.templateType === "mix") && <p>{addChannel}</p>}
                    </div>
                    <div className="button-wrap btnInfo">
                      {(data.templateType === "channel" ||
                        data.templateType === "mix") && (
                        <span className="btn">채널추가</span>
                      )}
                      {buttons.map((btn) => {
