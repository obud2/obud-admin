...
const DataListItem = ({
  ...
  useEffect(() => {
    const closeOption = (e) => {
      if (isOptionOpen && !optionRef.current?.contains(e.target))
  ...
  return (
    <SDataListItem
      className={`${isLoading ? "disabled" : ""} ${
        isSelect?.id && (isSelect?.id === data?.id ? "select" : "")
      }`}
    >
      ...
      {option?.map((o) => (
        <div
          className="data-list-field"
          key={`data-lite-item-${o?.label}`}
      ...
      {useOption?.length > 0 && (
        <div className="data-list-field">
          <button
            className="data-list-field-option"
            ref={optionRef}
          ...
          {isOptionOpen && (
            <ul className="data-list-option-container">
              {useOption?.map((item) => (
                <li
                  key={item?.label}
                  className="data-list-option-item"
                  onClick={() => item?.onClick(data?.id, data) || ""}
                >
                  {item?.label}
                </li>
              ))}
