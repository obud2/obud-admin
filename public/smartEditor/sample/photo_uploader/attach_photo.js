...
async function html5Upload() {
  ...
  for (var j = 0, k = 0; j < nImageInfoCnt; j++) {
    ...
    try {
      if (!!tempFile) {
        await setPhotoToEditor(tempFile);
        k += 1;
      }
    } catch (e) {}
    tempFile = null;
...
function callFileUploader() {
  oFileUploader = new jindo.FileUploader(jindo.$('uploadInputBox'), {
    sUrl: location.href.replace(/\/[^\/]*$/, '') + '/upload.js',
...
/**
	 ...
	 * ]
	 */
async function setPhotoToEditor(oFileInfo) {
  welLoading.className('loading active');

  try {
    const compressedFile = await imageOptimization(oFileInfo);

    const fileReader = new FileReader();
    fileReader.readAsDataURL(compressedFile);
    fileReader.onload = async (read) => {
      // S3 업로드처리
      const formData = new FormData();
      formData.set('files', compressedFile);

      const result = await axios.post(`${API_URL}/upload`, formData);
      console.log(result);

      const temp = {
        sFileName: result?.data.value.files[0].originalname || oFileInfo?.name,
        sFileURL: `${s3}${result?.data.value.files[0].bucket}/${result?.data.value.files[0].key}`,
        bNewLine: true,
      };

      if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
        opener.nhn.husky.PopUpManager.setCallback(window, 'SET_PHOTO', [[temp]]);
        welLoading.className('loading');
      }
      window.close();
    };
  } catch (error) {
